...
builder.Services.AddCors(opt =>
    opt.AddPolicy("AllowAngular", p =>
        p.WithOrigins("http://localhost:4242", "http://localhost:4200")
         .AllowAnyHeader()
         .AllowAnyMethod()));

var app = builder.Build();

// Auto-migrate on startup and initialize Hangfire jobs
using (var scope = app.Services.CreateScope())
{
    ...
    recurringJobManager.AddOrUpdate<OutboxPollerJob>(
        "outbox-poller",
