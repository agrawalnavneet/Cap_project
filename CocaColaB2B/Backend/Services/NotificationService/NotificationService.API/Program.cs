...
try
{
    ...
    builder.Host.UseSerilog((ctx, services, loggerConfig) =>
        SerilogConfiguration.ConfigureFromSettings(ctx, services, loggerConfig, "NotificationService"));
    ...
    builder.Services.AddDbContext<NotificationDbContext>(o => o.UseSqlServer(
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        catch (Exception ex)
        {
            ...
            Log.Warning("DB not ready (attempt {Attempt}/10): {Message}. Retrying in {Delay}ms", i + 1, ex.Message, delay);
            if (i == 9) throw;
