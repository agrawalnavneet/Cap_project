...
@Component({
  ...
  template: `
    <div class="page-container">
      <ccb2b-page-header title="Super Admin Overview" subtitle="Global system status and key metrics"></ccb2b-page-header>

      <!-- KPI Row 1 -->
      <div class="kpi-grid">
        <ccb2b-stat-card title="Total Staff" [value]="loading ? '' : staffCount" [loading]="loading" subtitle="Admin + Warehouse + Logistics" icon="users" iconColor="pink"></ccb2b-stat-card>
        <ccb2b-stat-card title="Total Dealers" [value]="loading ? '' : dealerCount" [loading]="loading" [subtitle]="pendingDealers + ' pending approval'" icon="store" iconColor="blue" [trend]="pendingDealers > 0 ? 'up' : 'neutral'"></ccb2b-stat-card>
        <ccb2b-stat-card title="Active Agents" [value]="loading ? '' : agentCount" [loading]="loading" icon="truck" iconColor="green"></ccb2b-stat-card>
        <ccb2b-stat-card title="Total Products" [value]="loading ? '' : productCount" [loading]="loading" icon="package" iconColor="purple"></ccb2b-stat-card>
      </div>

      <!-- KPI Row 2 -->
      <div class="kpi-grid" style="margin-top:14px">
        <ccb2b-stat-card title="Network Revenue" [value]="loading ? '' : '₹' + formatNum(networkRevenue)" [loading]="loading" subtitle="From all orders" icon="trending-up" iconColor="teal"></ccb2b-stat-card>
        <ccb2b-stat-card title="Total Orders" [value]="loading ? '' : totalOrders" [loading]="loading" [subtitle]="activeOrders + ' active'" icon="shopping-cart" iconColor="blue"></ccb2b-stat-card>
        <ccb2b-stat-card title="Delivered Orders" [value]="loading ? '' : deliveredOrders" [loading]="loading" icon="clipboard-list" iconColor="green"></ccb2b-stat-card>
        <ccb2b-stat-card title="System Uptime" [value]="'99.9%'" icon="activity" iconColor="teal" subtitle="All services healthy"></ccb2b-stat-card>
      </div>
