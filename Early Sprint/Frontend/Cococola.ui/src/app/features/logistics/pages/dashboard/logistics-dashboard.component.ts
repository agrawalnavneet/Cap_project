...
@Component({
  ...
  template: `
    <div class="page-container">
      <ccb2b-page-header title="Logistics Dashboard" subtitle="Monitor deliveries and agent assignments"></ccb2b-page-header>
      <div class="kpi-grid">
        <ccb2b-stat-card title="Pending Assignment" [value]="pendingCount" icon="truck" iconColor="amber" [loading]="loading"></ccb2b-stat-card>
        <ccb2b-stat-card title="Active Deliveries" [value]="activeCount" icon="map-pin" iconColor="blue" [loading]="loading"></ccb2b-stat-card>
        <ccb2b-stat-card title="Delivered Today" [value]="deliveredToday" icon="package" iconColor="green" [loading]="loading"></ccb2b-stat-card>
        <ccb2b-stat-card title="SLA At Risk" [value]="slaAtRisk" icon="clock" iconColor="red" [loading]="loading"></ccb2b-stat-card>
      </div>
      <!-- Map Placeholder -->
        ...
        <table *ngIf="recentShipments.length > 0" class="mini-table">
          ...
          <tbody>
            <tr *ngFor="let s of recentShipments.slice(0,10)">
              ...
              <td><ccb2b-status-badge [status]="s.status"></ccb2b-status-badge></td>
            </tr>
          </tbody>
        </table>
      </div>
