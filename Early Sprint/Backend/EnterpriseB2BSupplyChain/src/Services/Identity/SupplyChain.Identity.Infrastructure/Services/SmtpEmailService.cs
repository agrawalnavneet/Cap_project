...
/// share consistent Coca-Cola B2B branding (header, footer, palette, typography).
/// </summary>
public class SmtpEmailService : IEmailService
{
        ...
        string toEmail, string subject, string body, bool isHtml = false, CancellationToken ct = default)
    {
        ...
        // Otherwise, wrap plain text in a simple Coca-Cola B2B branded paragraph layout.
        var html = isHtml
            ? body
            : HulEmailLayout.Wrap(
                title:    subject,
        ...
        string toEmail, string subject, string htmlBody, CancellationToken ct, string fallbackContext)
    {
        ...
        var fromName    = _configuration["Email:FromName"]    ?? "Coca-Cola B2B";
        ...
        message.Headers.Add("X-Mailer",  "CocaCola B2B Platform");
    ...
    private static (string Title, string Intro) ResolveOtpCopy(string context)
    {
        ...
        return key switch
        {
            "dealer registration" => (
                "Verify Your Email",
                "Welcome to the Coca-Cola B2B Supply Platform! Please use the verification code below to complete your dealer registration."),
            "dealer login" => (
